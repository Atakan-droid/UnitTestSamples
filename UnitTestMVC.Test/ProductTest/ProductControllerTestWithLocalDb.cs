...
{
    public class ProductControllerTestWithLocalDb : ProductControleTest
    {
        public ProductControllerTestWithLocalDb()
        {
            setContextOptions(new DbContextOptionsBuilder<DbForUntTestContext>().UseSqlServer(@"Server=(localdb)\\MSSQLLocalDB;Database=TestDb;Trusted_Connection=true").Options);
            Seed();
        }
