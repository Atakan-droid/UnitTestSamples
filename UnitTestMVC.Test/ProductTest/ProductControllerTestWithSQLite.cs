...
{
    public class ProductControllerTestWithSQLite : ProductControleTest
    {
        public ProductControllerTestWithSQLite()
        {
            ...
            setContextOptions(new DbContextOptionsBuilder<DbForUntTestContext>()
        ...
        [Fact]
        public async Task Delete_AllProducts()
        {
            using (var context = new DbForUntTestContext(_contextOptions))
            {
            ...
            using (var context = new DbForUntTestContext(_contextOptions))
            {
                var products = context.Products.Where(x => x.CategoryId == 1);
